...
header('Content-Type: text/html; charset=utf-8;');
 ...
 class PDF extends FPDF
	...
	function contenido()
			...
			$rf=$fp[0].$fp[1].$fp[2];
			$this->MultiCell(0,5,utf8_decode("EL DÍA ".strtoupper(cambioFecha($rf)).' INCLUYENDO LO ESTABLECIDO EN HOJA ANEXA (VC01 REV. 0 COTIZACIÓN).'));
			$this->Ln(7);
			...
			$this->Cell(15,7,"",'C',0,1);
			$this->Cell(80,7,"CONCEPTO",1,0,'C');
...
//  VALIDACION DE FECHA EN CONFIGURACIONES DEPOSIO INICIAL Y ABONOS HECHOS
			...
			if($mostrar['fechas']==''){
				...
				if($i==$cantidad_abonos){
					...
					$fecha=date ( 'Y-m-j' , $nuevafecha );
					$this->Cell(15,7,"",'C',0,1);
					$this->Cell(80,7,"LIQUIDACION",1,0,'C');
					$this->Cell(40,7,$fecha,1,0,'C');
					$this->Cell(40,7,"$".round($pagos,2),1,0,'C');
				}else if($i<$cantidad_abonos){
					$this->Cell(15,7,"",'C',0,1);
					$this->Cell(80,7,$i." ABONO",1,0,'C');
					$this->Cell(40,7,$fecha,1,0,'C');
					$this->Cell(40,7,"$".round($pagos,2),1,0,'C');
				} 
				}
				...
				for($i=0;$i<$cantidad_abonos-1;$i++){
					$this->Ln(7);
					$this->Cell(15,7,"",'C',0,1);
					$this->Cell(80,7,$concepto[$i],1,0,'C');
			...
			$this->MultiCell(0,5,"FESTEJADO: ".$mostrar['festejado']);
