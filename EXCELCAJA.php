...
?>
 ...
 <?php
	...
	while($m=mysql_fetch_array($r)){
		if(mostrador($m['nomcontrato'])){
			echo "<tr><td>".$m['recibide']."</td><td>".$m['id']."</td><td>".$m['numcontrato']."</td><td>".$m['fechapago']."</td><td align='right'>0</td><td></td><td></td><td></td><td></td></tr>";
		}else{
	...
	while($m=mysql_fetch_array($r)){
		if(mostrador($m['nomcontrato'])){
			echo "<tr><td>".$m['recibide']."</td><td>".$m['id']."</td><td>".$m['numcontrato']."</td><td>".$m['fechapago']."</td><td>0</td><td></td><td></td><td></td><td></td></tr>";
		}else{
	...
	while($m=mysql_fetch_array($r)){
		if(mostrador($m['nomcontrato'])){
			echo "<tr><td>".$m['recibide']."</td><td>".$m['id']."</td><td>".$m['numcontrato']."</td><td>".$m['fechapago']."</td><td></td><td>0</td><td></td><td></td><td></td></tr>";
	...
	while($m=mysql_fetch_array($r)){
		if(mostrador($m['nomcontrato'])){
			echo "<tr><td>".$m['recibide']."</td><td>".$m['id']."</td><td>".$m['numcontrato']."</td><td>".$m['fechapago']."</td><td></td><td></td><td>0</td><td></td><td></td></tr>";	
		}else{
	...
	while($m=mysql_fetch_array($r)){
	if(mostrador($m['nomcontrato'])){
		echo "<tr><td>".$m['recibide']."</td><td>".$m['id']."</td><td>".$m['numcontrato']."</td><td>".$m['fechapago']."</td><td></td><td></td><td></td><td>0</td><td></td></tr>";
	}else{
	...
	while($m=mysql_fetch_array($r)){
	if(mostrador($m['nomcontrato'])){
		echo "<tr><td>".$m['recibide']."</td><td>".$m['id']."</td><td>".$m['numcontrato']."</td><td>".$m['fechapago']."</td><td></td><td></td><td></td><td></td><td>0</td></tr>";
		}else{
	...
	function traspasos(){
		...
		$ultima=mysql_query('SELECT MAX( fecha ) as f FROM saldos where fecha<"'.$hoy.'"');
		$ultima_fecha=mysql_fetch_array($ultima);
		$query=mysql_query('select * from traspasos where fecha<="'.$hoy.'" and fecha>="'.$ultima_fecha['f'].'"');
		echo "<tr><th>Tipo</th><th>Fecha</th><th>Cuenta origen</th><th>Cuenta destino</th><th>Cantidad</th></tr>";
		while($m=mysql_fetch_array($query)){
			echo "<tr><td>".tipotr($m['tipo'],$m['cuenta'],$m['cuenta2'],$m['efectivo'])."</td><td>".$m['fecha']."</td><td>".$m['cuenta']."</td><td>".$m['cuenta2']."</td><td>".$m['efectivo']."</td></tr>";
			
		}
	}
	function tipotr($t,$cuenta,$cuenta2,$c){
	global $efectivo;
		if($t==1){
			cuentas_egresos($cuenta,$c);
			cuentas($cuenta2,$c);
			return 'Entre Cuentas';
		}elseif($t==2){
			cuentas_egresos($cuenta,$c);
			$efectivo=$efectivo+$c;
			return 'Cuenta a Efectivo';
		}else{
			$efectivo=$efectivo-$c;
			cuentas($cuenta,$c);
			return 'Efectivo a Cuenta';
		}
	}
	?>
 </head>
...
<center>
		...
		<table >
			...
			<table border='1'>
				<tr><th colspan='5' align='center'>Traspasos</th></tr>
				<?php traspasos();?>
			</table>
			<table>
			<tr></tr>
