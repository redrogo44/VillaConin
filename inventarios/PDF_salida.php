...
class PDF extends FPDF{
	function Header(){
		...
		$this->Cell(80);
		// Título
		$this->Cell(30,10,"Nota de salida",0,0,'C');
		// Salto de línea
		$this->Ln(20);
		}
	function Footer(){
	...
	function cuerpo(){
			...
			$entrada=mysql_query("select * from salida where id_salida=".$_GET['folio']);
			$m_entrada=mysql_fetch_array($entrada);
			$queryi=mysql_query("select * from detalle where id=".$_GET['folio']." and tipo='salida'");
			}else{
				die('ERROR AL GENERAR DOCUMENTO variable: ' . mysql_error());
			}

			$this->Cell(30,5,"Folio",1,0,'C');
			$this->Cell(30,5,"Fecha de salida",1,0,'C');
			$this->Ln(5);
			///////////serie
			$this->Cell(30,5,$m_entrada['id_salida'],1,0,'C');
			$this->Cell(30,5,$m_entrada['fecha'],1,0,'C');
			$this->Ln(10);
			$this->Cell(15,5,"cantidad",1,0,'C');
			$this->Cell(140,5,"producto",1,0,'C');
			$this->Cell(17,5,"precio",1,0,'C');
