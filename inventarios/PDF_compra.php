...
class PDF extends FPDF{
	...
	function cuerpo(){
			...
			while($producto=mysql_fetch_array($produc)){
				...
				$this->Cell(110,5,utf8_decode($p2['nombre'].'('.$p2['descripcion'].')'),1,0,'L');
				...
				*/
				...
				$this->Cell(50,5,utf8_decode("Realizó"),T,0,'C');												//////cancelacion				
			if($_GET['cancelacion']==1){
				//$this->Image('imagenes/cancelado.png' , 40 ,40, 120 , 70);	
				if($_GET['type']=='si'){
					///validamos que ya se halla registarado en la tabla de cancelaciones
					$VR=mysql_query("select * from cancelacion where id=".$_GET["folio"]." and tipo='comprafac'");
					if(mysql_num_rows($VR)==0){
							$insert=mysql_query("insert into cancelacion(id,tipo,fecha) values(".$_GET['folio'].",'comprafac','".date('Y-m-d')."')");
							$update=mysql_query("update detalle set tipo='comprafac-cancelada' where id=".$_GET['folio']." and tipo='comprafac'");	
					}	
				}else{
					$VR=mysql_query("select * from cancelacion where id=".$_GET["folio"]." and tipo='compra'");
					if(mysql_num_rows($VR)==0){
						$insert=mysql_query("insert into cancelacion(id,tipo,fecha) values(".$_GET['folio'].",'compra','".date('Y-m-d')."')");	
						$update=mysql_query("update detalle set tipo='compra-cancelada' where id=".$_GET['folio']." and tipo='compra'");	
					}
				}			
			}
