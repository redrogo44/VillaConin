...
<html lang="en" class="no-js">
	<head>
		...
		<script>
		function deshabilitar_next(){
			document.getElementById('sig_etp_1').disabled=true;
		}
					...
					select : function(event, ui){
                       $('#description').slideUp('fast', function(){
                            $('#description').html(
                                ...
                                '<strong>Impuesto: </strong>' + ui.item.impuesto+'<br/>'+
                                '<strong>Ultimo Costo: </strong>' + ui.item.uc
                            );
                       });
	...
	<div class="container">
			...
			if(!isset($_POST['etapa'])){
				...
				<div id="formulario">
					...
					<table>
					<tr><td><h2>Proveedor</h2></td><td align="bottom"><input type="text" id="proveedor" name="proveedor"  onkeyup="deshabilitar_next();" required><img src="imagenes/agregar.png"  width="20px" height="20px" onclick="nuevo_proveedor()"></td></tr>
					<tr><td colspan="2" align="center"><input id="sig_etp_1" type="submit" name="opcion" value="Siguiente" disabled="true"></td></tr>
				...
				if(isset($_POST['lista_cantidad']) && $_POST['lista_cantidad']!=''){
					for($i=0;$i<count($lcantidad)-1;$i++){
						if($lproducto[$i]!=''){
							...
							echo "<td>".$lgasto[$i]."</td>";
							echo "</tr>";
							$mostrar_cantidad=$mostrar_cantidad+$lcantidad[$i];
							$t=$t+($lcosto[$i]*$lcantidad[$i]);
							$ti=$ti+(($lcosto[$i]*$lcantidad[$i])*($limpuesto[$i]/100));
						}
					}
					echo "<tr><td colspan='2' align='right'>Total</td><td>".$mostrar_cantidad."</td><td colspan='2' align='right'>SubTotal</td><td>$".round($t,5)."</td><td></td><td></td></tr>";
					echo "<tr><td colspan='5' align='right'>Impuesto</td><td>$".round($ti,5)."</td><td></td><td></td></tr>";
					echo "<tr><td colspan='5' align='right'>Total</td><td>$".round($ti+$t,5)."</td><td></td><td></td></tr>";
				}			
			echo '</table>
			</div><br><br>';			
			////////nuevo registro
