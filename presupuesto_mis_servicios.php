...
if($_POST['id']!='xdx' && !isset($_POST['op'])){////////////lo ejecutamos su solo es no es solo para mostrar los servicios contratados
	...
	$bandera=false;
	$up=mysql_fetch_array(mysql_query("select * from presupuesto where id_precliente='".$_POST['contrato']."'"));
	...
	if($bandera){////////es verdadera si ya existia el servicio
		...
		$q=mysql_fetch_array(mysql_query("select * from presupuesto where id_precliente='".$_POST['contrato']."'"));
		if($q['servicios']!=''){			
			mysql_query("update presupuesto set servicios='".$q['servicios']."%".$_POST['id']."-".$_POST['hrs']."' where id_precliente='".$_POST['contrato']."'");
		}else{			
			mysql_query("update presupuesto set servicios='".$_POST['id']."-".$_POST['hrs']."' where id_precliente='".$_POST['contrato']."'");
		}
	}
	...
	for($i=0;$i<count($servicios_up);$i++){
		if($i==$_POST['id']){
			...
			if($nuevo_contenido==""){
				$nuevo_contenido=$s_up[0]."-".$_POST['hrs']."-".$s_up[2].$ext;
			}else{
				$nuevo_contenido=$nuevo_contenido."%".$s_up[0]."-".$_POST['hrs']."-".$s_up[2].$ext;
			}
		}else{
			if($nuevo_contenido==""){
	...
	/////actualizamos
	mysql_query("update presupuesto set servicios='".$nuevo_contenido."' where id_precliente='".$_POST['contrato']."'");
...
}else if($_POST["op"]=='mod_precio'){
	mysql_query("update presupuesto set precio=".$_POST["precio"]." where id_precliente=".$_POST["id"]);
}

$q2=mysql_fetch_array(mysql_query("select * from presupuesto where id_precliente='".$_POST['contrato']."'"));
