...

{
		...
		{				
						$q="SELECT id FROM abono Where id=".$_GET['id'];
						$r=mysql_query($q);
						while($muestra=mysql_fetch_array($r))
						{
							$numax=$muestra['id'];
						}
						$q2="SELECT *  FROM abono where id=".$_GET['id'];
						$r2=mysql_query($q2);
...

						{
						   ...
						   $recibide=utf8_decode($muestra2['recibide']);

						   $cantidadde=$muestra2['cantidad'];

						   $concepto=utf8_decode($muestra2['concepto']);
						...
						$this->Cell(130,6.8,$recibide,0,1,'C');
