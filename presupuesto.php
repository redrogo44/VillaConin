...
/////////validamos la existencia del precliente en la tabla de presupuestos
$ep=mysql_query("select * from presupuesto where id_precliente='".$_GET['numero']."'");
$cep=mysql_num_rows($ep);
if($cep==0){
    mysql_query("insert into presupuesto(id_precliente) values('".$_GET['numero']."')");
}
?>
...
xmlhttp.open("POST","info_service.php",true);////ya se verifico
...
function agregar3(id,hrs){
     var xmlhttp;
        ...
        xmlhttp.open("POST","presupuesto_mis_servicios.php",true);
        xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			...
			if (xmlhttp.readyState==4 && xmlhttp.status==200){
				...
				if(recibido2[0]=='HORA' || recibido2[0]=='PIEZA'){
                    ...
                    if(cantidad!=null && cantidad>0){
                     ...
                     if(or=='#'){////validamos si es un servicio opcional							alert(xmlhttp.responseText);
                          agregar2(id,recibido2[0]);
                     }else{
                         agregar3(id,recibido2[0]);
                     }
...
function Finalizar(){ 
	...
	if(confirm("¿Esta seguro de generar el presupuesto?")){
		window.location.href = 'CreaPresupuesto.php?precliente='+contrato+"&comentario="+c;
	}
}
...
function mod_pc(){
	p=document.getElementById("precio_comensal").value;
	//alert(p);
	var xmlhttp;
		if (window.XMLHttpRequest){// code for IE7+, Firefox, Chrome, Opera, Safari
			xmlhttp=new XMLHttpRequest();
		...
		xmlhttp.open("POST","presupuesto_mis_servicios.php",true);
		xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		xmlhttp.send("op=mod_precio&id=<?php echo $_GET["numero"];?>&precio="+p);
}
...
-->
<!-- CUERPO DEL WEB-->
<body background="../Imagenes/Villa Conin.png" style="background-repeat:no-repeat"  bgcolor="#FFFFFF" onload='agregar2("xdx",0,0)'>
...
<?php
	...
	echo "</div>";
	echo "<div id='observaciones' class='observaciones'>";	$m_ep=mysql_fetch_array($ep);
	echo "OBSERVACIONES<br><br><textarea id='comment' style='text-indent:0px;width:150px;height:100px;'>".$m_ep['comentario']."</textarea>";
	echo "<br><br>";
	echo "Precio por comensal<br>";
	echo "<input type='text' id='precio_comensal' onchange='mod_pc()' value='".$m_ep["precio"]."'>";
	echo "</div>";
?>
