...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$Ara='SELECT * FROM contrato WHERE Fecha >= "'.$_POST['fecha1'].'" AND Fecha <= "'.$_POST['fecha2'].'" AND estatus=1 ORDER BY Fecha';
		...
		$i=0;
		$eventor[100][100];
 		$Ara3='SELECT * FROM Eventos_Recaudacion WHERE fecha >= "'.$_POST['fecha1'].'" AND fecha <= "'.$_POST['fecha2'].'" AND estatus="ACTIVO" ORDER BY fecha';
		$Fec3=mysql_query($Ara3);
		while ($Fecas3=mysql_fetch_array($Fec3)) 
		{
			
				if(empty($Array_Contratos)&&(strlen($Fecas3['Numero']))<9)
				{
					$eventor[$i][0]=$Fecas3['Numero'];
					$eventor[$i][1]=$Fecas3['fecha'];
					$i++;
				}
				else if(strlen($Fecas3['Numero'])<9)
				{
					$eventor[$i][0]=$Fecas3['Numero'];
					$eventor[$i][1]=$Fecas3['fecha'];
					$i++;
		...
		while(strtotime($first)<=strtotime($last))
		{
			...
			while(strtotime($eventor[$z][1])==strtotime($first)){
				if(empty($Array_C)){
					$Array_C=$eventor[$z][0];
				}else{
					$Array_C=$Array_C.','.$eventor[$z][0];
				}
				$z++;
			}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	?>
	  <div id="dialog" title="Meseros que no llamaron" style="display:none;">
	            			...
	            			// CONTRATOS
			               		...
			               		{ 
			               			...
			               			if (mysql_num_rows($Se)<1) 
			               			{
										...
										{
											$Ser=mysql_query("SELECT * FROM Eventos_Recaudacion WHERE Numero='".$array[$j]."'");				
											$C=mysql_fetch_array($Ser);
										}
										else
										{
							...
							$bandera=0;
			               		...
			               		{ 
			               			...
			               			if (mysql_num_rows($Se)<1) 
			               			{
										...
										{
											$Ser=mysql_query("SELECT * FROM Eventos_Recaudacion WHERE Numero='".$array[$j]."'");
											$C=mysql_fetch_array($Ser);$bandera=1;
										}
										else
			               				{$C=mysql_fetch_array($See);}
			               			}
									...
									if($bandera==1)
									{echo "<td align='center' bgcolor='#C9EED0'><h5><b>Recaudacion</b></h5></td>";}
								if($bandera==0){echo "<td align='center' bgcolor='#C9EED0'><h5><b>".$C['tipo']."</b></h5></td>";	}
									$bandera=0;
			               		}
	               	  echo "</tr>
	               	  		<tr>";
			               		...
			               		{ 
			               			$esfac=mysql_query("SELECT * FROM contrato WHERE Numero='".$array[$j]."'");
										...
										if (mysql_num_rows($efac)<1) 
			               		 		{
											$efar=mysql_query("SELECT * FROM Eventos_Recaudacion WHERE Numero='".$array[$j]."'");
											$rr=mysql_fetch_array($efar);
											echo "<td style='color:#0015FF' align='center' bgcolor='#FF9F63'><h6><b>Total de<br> Comensales = ".$rr['comensales']."</b></h6></td>";	
										}
										else
										{
	               	  		...
	               	  		<tr>";
			               		...
			               		{ 
									...
									if(mysql_num_rows($S)<1)
									{
										...
										{
											$ssr=mysql_query("SELECT * FROM Eventos_Recaudacion WHERE Numero='".$array[$j]."'");
											$sr=mysql_fetch_array($ssr);
											echo "<td width='140px' style='color:#000' align='center' bgcolor='#B2B4FC'><h6><b>".$sr['salon']."</b></h6></td>";
										}
										else
										{
	               	  		...
	               	  		<tr>";
			               		...
			               		{ 
									...
									if (mysql_num_rows($Z)<1) 
									{
										...
										if(mysql_num_rows($SA)<1)
										{$SAr=mysql_query("SELECT * FROM Eventos_Recaudacion WHERE Numero='".$array[$j]."'");
											$zzr=mysql_fetch_array($SAr);$bandera=1;
										}
										else
	               	  		...
	               	  		<tr>";
			               		...
			               		{ 			               			
								 	...
								 	if(mysql_num_rows($Z)<1)
								 	{
										...
										if(mysql_num_rows($za)<1)
										{
											$zr=mysql_query("SELECT * FROM Eventos_Recaudacion WHERE Numero='".$array[$j]."'");
											$zz=mysql_fetch_array($zr);$bandera=1;
										}
										else{$zz=mysql_fetch_array($za);}
								 	}
								 	else
								 	{							 		
								 		$zz=mysql_fetch_array($Z);
									...
									if(mysql_num_rows($Com)<1)
									 	{
											...
											if(mysql_num_rows($za)<1)						
											{
												$zar=mysql_query("SELECT * FROM Eventos_Recaudacion WHERE Numero='".$array[$i]."'");
												$zzr=mysql_fetch_array($zar);														
												$PROM=$zzr['comensales']/20;
	...
	<div id="tres" class="tres" onscroll="vertical(this)">
			<table  border="2" border-color="#640404" background-color="fff" id="tabla2" style="overflow=auto">
						<tr>
    		      		...
    		      		$NCheck=0;
    		      	 $ORDEN=OrdenaMeseros();    		      		
    		      		$Selec=mysql_query("SELECT * FROM Meseros WHERE disponibilidad='si' group by ".$ORDEN);
	...
	<table  border-color="#640404" background-color="fff" id="tabla2" style="overflow=auto" border="2">';
				echo "<tr>";
    		      	 ...
    		      	 $ORDEN=OrdenaMeseros();
			$rrr="SELECT * FROM Meseros WHERE disponibilidad='si' group by ".$ORDEN;
	$Selec=mysql_query($rrr);
    		      		while ($M=mysql_fetch_array($Selec)) 
    		      		{
    		      			Echo "
    		      				<tr>
			    		      			...
			    		      			{    echo "<tr>";  		# code...			    		      			
				    		      			for ($i=0; $i < count($array); $i++) 
												...
												$ex=explode("-",$zz['Fecha']);
												 ...
												 {$Diaa="Sabado"; $color="#090A0A";} // NEgro
											$ti='"'.$Me['tipo'].'"'	;
												echo "<td width='140px' height='60px' bgcolor=".$color." align='center'><input type='checkbox' id='".$NCheck."' name='".$NCheck."' value=".$NCheck.",".$Me['id'].",".$Me['tipo']."  onchange='Cuenta();' title='".$NCheck.",".$Me['id']."'/></td>"; $NCheck++;
	...
	for($i=0;$i<count($array);$i++){
	 ...
	 for(j=0;j<<?php echo $NNN; ?>;j++){
	 	if(document.getElementById(j).checked){
	 		...
	 		//alert(t[2]);
	 		if (t[2]=='MESERO_2'||t[2]=='MESERO_1'||t[2]=='MESERO_ESTRELLA') 
	 		{
		 		<?php
		 			for($i=0;$i<count($array);$i++){
...
function abrir_dialog() {
 	//alert(tipo);
var contratos="<?php echo $Array_C;?>";
//alert(contratos);
 	var xmlhttp;
	...
	xmlhttp.onreadystatechange=function()
  	{
		...
		xmlhttp.open("POST","https://villaconin.mx/Config/detalle_meseros_llamada.php",true);
		xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		xmlhttp.send("contratos="+contratos);
