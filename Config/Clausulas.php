...
<?php
conectar();
$r=mysql_query('select * from clausulas');

if(isset($_GET['id'])){
	$r2=mysql_query('select * from clausulas where id='.$_GET['id']);
}else{
	$r2=mysql_query('select * from clausulas where id=1');
}
...
<form action="MClausulas.php" name="f" method="POST">
<label>Clausulas</label>
<select name='num' onchange="location.href='Clausulas.php?id='+f.num.value">
<option></option>
<?php
