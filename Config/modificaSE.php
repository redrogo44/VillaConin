...
$ns1=mysql_fetch_array(mysql_query("select * from Servicios_categorias where nombre='MANTELERIA'"));
$ns2=mysql_fetch_array(mysql_query("select * from Servicios_categorias where nombre='MOBILIARIO EVENTOS'"));
$ns3=mysql_fetch_array(mysql_query("select * from Servicios_categorias where nombre='PEWTER EVENTOS'"));
$ns4=mysql_fetch_array(mysql_query("select * from Servicios_categorias where nombre='TEMATICA EVENTOS'"));
...
$datos=mysql_fetch_array(mysql_query("select * from Servicios where id=".$_GET['numero']));
if($datos['tipo']!=$ns1['id'] && $datos['tipo']!=$ns2['id'] && $datos['tipo']!=$ns3['id'] && $datos['tipo']!=$ns4['id']  && $datos['tipo']!=$ns5['id'] ){
    $link="location.href='Conf_Servicios.php'";
    echo '<!--ESTILO CUERPO-->
<div align="center"  >
 
   <!-- Tabala Pre-Registro -->
   <h1>MODIFICAR SERVICIO</h1>
   <p><button class="button" onclick="'.$link.'">Regresar</button></p>
   <br />
    <form action="alta_servicio.php" method="post" id="mod_ser" onsubmit="return valida_form()">
    <table border="9"   bordercolor="#990000" bordercolordark="#990000"	 bordercolorlight="#990000" >
       <tr>
          <td><b>SERVICIO</b></td>
           <td><input id="name_s" name="servicio" type="text" size="35" maxlength="35" value="'. $datos['Servicio'].'" onchange="comprobar(this.value)" required></td>
       </tr>
       <tr> 
        <td><b>DESCRIPCION</b></td> 
        <td><textarea name="descripcion"  required="required">'.$datos['descripcion'].'</textarea>
      </tr>
    <tr>
       <td><b>TIPO</b></td>
			<td>    
    ...
    <tr>
        ...
        <td ><input type="text" name="precio" required="required" value="'.$datos['precio'].'"/> </td>
    </tr>
    <tr>
    	<td><b>UNIDAD DE MEDIDA</b></td>
        <td ><select name="unidad" size="1" id="unidad" >
			<option value="'.$datos['unidad'].'">'.$datos['unidad'].'</option>
		...
		<!--label id="env_ser"  class="button" onclick="enviar_form()"/>Enviar</label-->
       </td>
       </tr>
   </table>
    ';
    ...
    $catx=mysql_fetch_array(mysql_query("select * from Servicios_categorias where id=".$datos['tipo']));
    echo '<!--ESTILO CUERPO-->
<div align="center"  >
   <!-- Tabala Pre-Registro -->
   <h1>MODIFICAR SERVICIO</h1>
       <p><button class="button" onclick="back();">Regresar</button></p> 
   <br />
    <form action="alta_servicio.php" method="post" name="newregist" onsubmit="return sure()">
    <table border="9"   bordercolor="#990000" bordercolordark="#990000"	 bordercolorlight="#990000" >
       <tr>
          <td><b>SERVICIO</b></td>
           <td><input name="servicio" type="hidden" size="35" maxlength="35" value="'. $datos['Servicio'].'">'. $datos['Servicio'].'</td>
       </tr>
       <tr> 
        <td><b>DESCRIPCION</b></td> 
        <td><textarea name="descripcion"  required="required">'.$datos['descripcion'].'</textarea>
    ...
    <tr>
       <td><b>TIPO</b></td>
			<td>
            <input type="hidden" name="categoria" value="'.$datos['tipo'].'">
			 '.$catx['nombre'].'
			</td>
    </tr>
    ...
    <tr>
    	<td><b>UNIDAD DE MEDIDA</b></td>
        <td ><input type="hidden" name="unidad" value="'.$datos['unidad'].'">
        '. $datos['unidad'].'</td>
    </tr> 
        <tr>
         <td colspan="2" align="center"><br>
   	    <input type="hidden" name="tipo" value="Modificar"/>
    	<input type="submit" name="boton"  value="Enviar" class="button" />
        <input type="hidden" name="id" value="'.$_GET['numero'].'"/>
       </td>
       </tr>
   </table>    
...
function cargar_categorias(){
    var xmlhttp;
            ...
            xmlhttp.send("opcion=show_modificar&tipo=<?php echo $datos["tipo"];?>");
}
...
function back(){
	location.href="Conf_Servicios.php";
}
</script>
</body>
</html>