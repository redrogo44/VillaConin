...
if($m['impreso']=='si'){
	echo "<META HTTP-EQUIV='Refresh' CONTENT='0; URL=MSaldo.php?numero=".$n."'>";
}
...
echo '<div id="style1"class="style1">';
			...
			if(c_gral($m['Numero'])){
				$si=totalsub($m['Numero']);
			}else{
				$si=pagoideal($m['Numero'],$m['facturado']);	
			}
			if($meses>0){
				$x=explode('%',$m['fechas']);
				$ideal=round(($si/($meses)),2);
				echo "<tr><td><label><font size='4'>Pago ideal</font></label></td><td><font size='4'>$".$ideal."</font></td></tr>";
			}else{
				echo "<tr><td><label><font size='4'>Pago ideal</font></label></td><td><font size='4'>".$si."</font></td></tr>";
			}
...
function pagoideal($NumeroContrato,$fac){
...
function c_gral($n){
	$q="select count(*) as total from contrato where Numero like '".$n."%'";
	$r=mysql_query($q);
	$m=mysql_fetch_array($r);
	if($m['total']==1){
		$b=false;
...
function totalsub($n){
	$q="select sum(si) as total from contrato where Numero like '".$n."-%'";
	$r=mysql_query($q);
	$m=mysql_fetch_array($r);
	return $m['total'];
}
?>
<form action='fechas_contrato.php' method='POST' onsubmit='return error1()'>
	...
	<table border='1'>
		...
		$si=$m['si'];
			if($_POST['opcion']=='agregar'){
				...
				insertar($n,$_POST);
				echo "<script>";
				echo "location.href='fechas_contrato.php?numero=".$_POST['numero']."'";
				echo "</script>";
...
function mostrar($n,$fecha,$concepto,$monto,$saldo_inicial){
	...
	echo "<form action='fechas_contrato.php' method='POST'>";
