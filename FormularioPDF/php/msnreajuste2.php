...
function principal($numero){
		...
		$msj=$msj."<br>Numero :".$m['Numero']."<br>Nombre :".$m['nombre']."<br>";
		$msj=$msj. "Saldo inicial: $".$m['si']."<br>";
		$msj=$msj. "Cargos: $".cargo($m['Numero'])."<br>";
		...
		$msj=$msj."Fecha del Evento: ".$m['Fecha']."<br><br><br>";	
		if($m['facturado']=='si'){
			$sentencia="select max(id) as d from abonofac";
			$sentencia2=mysql_query($sentencia);
			$sentencia3=mysql_fetch_array($sentencia2);
			$abono_c="select * from abonofac where id=".$sentencia3['d'];
			$abono_c2=mysql_query($abono_c);
			$abono_c3=mysql_fetch_array($abono_c2);
		}else{
			$sentencia="select max(id) as d from abono";
			$sentencia2=mysql_query($sentencia);
			$sentencia3=mysql_fetch_array($sentencia2);
			$abono_c="select * from abono where id=".$sentencia3['d'];
			$abono_c2=mysql_query($abono_c);
			$abono_c3=mysql_fetch_array($abono_c2);
		}
		...
		$msj=$msj."Monto del abono realizado: ".$abono_c3['cantidad']."<br><br><br>";	
	...
	if($cargo>0){
		...
		if($fechas[count($fechas)-2]<date('Y-m-d')){
			if($sumamontos<=abono($mostrar['Numero'])){////imprimimos de colores segun el avance del pago de abonos y cargo
				if($sumamontos+$cargo<=abono($mostrar['Numero'])){
					$msj=$msj."<td style='background:#FFCC99;'>Fecha<br>".$fechas[count($fechas)-2]."<br>Concepto<br>Cargos<br>$".$cargo."<Faltante><br>$0<br>Pagado<br>$".$cargo."</td>";
				}else{
					$resta=$sumamontos+$cargo-abono($mostrar['Numero']);
					$p=$cargo-$resta;
					$msj=$msj."<td style='background:#99FF99;'>Fecha<br>".$fechas[count($fechas)-2]."<br>Concepto<br>Cargos<br>Faltante<br>$".$resta."Pagado<br>$".$p."</td>";
				}
			}else{
